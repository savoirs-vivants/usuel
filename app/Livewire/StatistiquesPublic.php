...
class StatistiquesPublic extends Component
{
    ...
    {
        $this->availableAges = Beneficiaire::whereNotNull('age')->distinct()->pluck('age')->toArray();
        $this->availableGenres = Beneficiaire::whereNotNull('genre')->distinct()->pluck('genre')->toArray();
        $this->availableCsps = Beneficiaire::whereNotNull('csp')->distinct()->pluck('csp')->toArray();
        $this->availableDiplomes = Beneficiaire::whereNotNull('diplome')->distinct()->pluck('diplome')->toArray();
    ...
    {
        $this->reset(['selectedAges', 'selectedGenres', 'selectedCsps', 'selectedDiplomes', 'timeRange']);
        $this->dispatch('update-charts', data: $this->chartData);
    }
    ...
    public function chartData(): array
    {
        $query = Passation::query()
            ...
            ->select('passations.*', 'beneficiaires.age', 'beneficiaires.genre', 'beneficiaires.csp');
        ...
        match ($this->timeRange) {
            ...
            'Custom' => $query->whereBetween('passations.created_at', [$this->customStartDate, $this->customEndDate]),
        };
        ...
        if ($totalPassations > 0) {
            ...
            foreach ($passations as $passation) {
                $scoreArray = is_string($passation->score) ? json_decode($passation->score, true) : $passation->score;
    ...
    public function render()
    {
        ...
        return view('livewire.statistiques-public', [
            'labelsMap' => $labelsMap
        ]);
    }
}

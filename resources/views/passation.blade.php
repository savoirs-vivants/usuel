...
@section('content')
    ...
    <section class="flex min-h-screen bg-gray-50" x-data="selectionTable({{ collect($passations->items())->pluck('id')->toJson() }})">
        <div class="ml-64 flex-1 p-8">
            ...
            <div class="mb-4">
                ...
                <form action="{{ route('passations') }}" method="GET"
                    ...
                    <div class="flex items-center gap-2 w-full lg:max-w-md">
                        <div class="relative flex-1">
                            ...
                            @if (!empty($search))
                                <a href="{{ route('passations', ['per_page' => $perPage]) }}"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-red-500 bg-gray-50 rounded-full p-1 transition-colors"
                        ...
                        <button type="submit"
                            class="bg-sv-blue hover:bg-sv-blue/90 text-white px-5 py-2.5 rounded-2xl text-sm font-bold shadow-sm transition-colors shrink-0">
                            Rechercher
                        </button>
                    </div>
                    <div class="flex items-center gap-4">
                        ...
                        <div
                            class="flex items-center gap-2 bg-white px-4 py-2 border-2 border-gray-200 rounded-2xl shadow-sm shrink-0">
                            <label for="per_page" class="text-xs font-bold text-gray-400 uppercase tracking-wide">Nombres
                                par page :
                            </label>
            ...
            <div x-show="confirmBulkDelete" x-cloak
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
                    ...
                    <div class="flex gap-3">
                        ...
                        <form action="{{ route('passation.destroyMultiple') }}" method="POST" class="flex-1"
                            id="bulk-delete-form">
                            @csrf
                            @method('DELETE')
                            ...
                            <tr class="border-b-2 border-gray-100">
                                ...
                                <th class="px-5 py-4 bg-gray-50/70 w-10">
                                    <button @click="toggleAll()"
                                        ...
                                        :class="allSelected ? 'bg-sv-blue border-sv-blue' : (someSelected ?
                                            'bg-sv-blue/20 border-sv-blue' :
                                            'border-gray-300 hover:border-sv-blue bg-white')">
                                        <svg x-show="allSelected" class="w-3 h-3 text-white" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3"
                                                d="M5 13l4 4L19 7" />
                                        </svg>
                                        <svg x-show="someSelected && !allSelected" class="w-3 h-3 text-sv-blue"
                                ...
                                @if (auth()->user()->role == 'admin')
                                    <th
                                        class="text-left px-5 py-4 font-bold text-sv-blue text-xs uppercase tracking-wider bg-gray-50/70">
                                        ID</th>
                                @endif
                                @if (auth()->user()->role !== 'admin')
                                    <th
                        ...
                        <tbody>
                            @foreach ($passations as $passation)
                                <tr class="border-b border-gray-50 transition-colors duration-100"
                                    :class="selected.includes({{ $passation->id }}) ? 'bg-blue-50/60' :
                                        'hover:bg-gray-50/60'"
                                    ...
                                    <td class="px-5 py-4">
                                        <button @click="toggle({{ $passation->id }})"
                                            class="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-150 shrink-0"
                                            :class="selected.includes({{ $passation->id }}) ? 'bg-sv-blue border-sv-blue' :
                                                'border-gray-300 hover:border-sv-blue bg-white'">
                                            <svg x-show="selected.includes({{ $passation->id }})"
                                    ...
                                    @if (auth()->user()->role == 'admin')
                                        <td class="px-5 py-4">
                                            <span
                                                class="font-mono font-bold text-gray-400 text-xs bg-gray-100 px-2 py-1 rounded-lg">#{{ $passation->id }}</span>
                                        </td>
                                    @endif
                                    ...
                                    @if (auth()->user()->role !== 'admin')
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-3">
                                                <div
                                                    class="w-8 h-8 rounded-xl bg-sv-blue/10 flex items-center justify-center shrink-0 font-bold text-sv-blue text-xs uppercase">
                                    ...
                                    <td class="px-5 py-4">
                                        <div class="flex items-center gap-2 text-gray-600">
                                            ...
                                            <span
                                                class="font-medium text-sm">{{ $passation->date ? $passation->date->format('d/m/Y') : '—' }}</span>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-5 py-4">
                                        ...
                                        <div class="flex items-center gap-2.5">
                                            <span
                                                class="font-mono font-bold text-sv-blue text-sm">{{ $score }}</span>
                                    ...
                                    <td class="px-5 py-4">
                                        ...
                                        <div x-show="confirmDelete" x-cloak class="flex items-center gap-2">
                                            ...
                                            <form action="{{ route('passation.destroy', $passation) }}" method="POST"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="text-xs font-bold text-white bg-red-500 hover:bg-red-600 rounded-xl px-3 py-2 transition-colors">Oui</button>
                                            </form>

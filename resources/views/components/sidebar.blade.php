<aside class="w-60 shrink-0 bg-sv-blue flex flex-col fixed top-0 left-0 h-screen z-20">
    ...
    <nav class="flex-1 px-3 py-6 space-y-1">
        ...
        <a href="#"
            class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-150
            {{ Route::is('certifications.*') ? 'bg-sv-green text-white shadow-lg shadow-sv-green/20 font-bold' : 'text-white/70 hover:bg-white/10 hover:text-white font-semibold' }}">
            <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="mt-auto pt-4 border-t border-white/10" x-data="{ open: false }">
            ...
            <button @click="open = !open" @click.outside="open = false"
                class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-white/10 transition-all duration-150">
                <div
                    class="w-9 h-9 rounded-full bg-sv-green/80 flex items-center justify-center shrink-0 uppercase text-white font-bold text-xs">
            ...
            <div x-show="open" x-cloak x-transition:enter="transition ease-out duration-150"
                ...
                class="mt-1 bg-white/10 rounded-xl overflow-hidden border border-white/10">

                <a href="{{ route('profile.edit') }}"
                    class="flex items-center gap-3 px-4 py-2.5 text-sm text-white/80 hover:bg-white/10 hover:text-white transition-colors">

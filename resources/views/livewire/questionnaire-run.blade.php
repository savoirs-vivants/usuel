<div
    ...
    x-data="{
        ...
        init() {
            // Se lance au premier chargement de la page
            this.startTracker();
        },

        startTracker() {
            // Nettoie l'ancien tracker s'il existe
            if (this.tracker) { this.tracker.destroy(); this.tracker = null; }

            // Va chercher les infos de la question en cours directement dans le HTML
        ...
        async validerQ() {
            // 1. Récupère les stats et tue le tracker
            const data = this.tracker ? this.tracker.collect() : {};
            if (this.tracker) { this.tracker.destroy(); this.tracker = null; }

            // 2. Envoie à Livewire et ATTEND qu'il finisse de charger la nouvelle question
            await this.$wire.validerAvecTracking(data);

            // 3. Une fois le HTML mis à jour, on relance le tracker pour la question suivante
            this.$nextTick(() => {
                this.startTracker();
            });
        },

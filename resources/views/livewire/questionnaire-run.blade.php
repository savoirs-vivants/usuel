<div class="w-full max-w-4xl mx-auto px-6 pb-20">
    ...
    {{-- Modification ici : On utilise la variable injectée par le render() --}}
    @if ($currentQuestion)

        <div class="flex gap-1 mb-10 w-full mx-auto">
            @for ($i = 0; $i < $totalQuestions; $i++)
                <div
                    class="h-1.5 flex-1 rounded-full transition-colors duration-300 {{ $i <= $currentIndex ? 'bg-[#1a9e7e]' : 'bg-gray-200' }}">
                </div>
            @endfor
        </div>
        ...
        <div class="space-y-4 max-w-2xl mx-auto mb-10">
            ...
            @foreach ($currentQuestion->choixSansE as $lettre => $choixData)
                <button
                    {{-- Le wire:key pour forcer la mise à jour visuelle des boutons --}}
        ...
        <div class="flex justify-center gap-4">
            ...
            <button wire:click="valider" @if ($selectedAnswer === '') disabled @endif
                class="px-8 py-3 rounded-xl font-bold text-white transition-all shadow-md w-48
        {{ $selectedAnswer === ''
            ? 'bg-gray-300 cursor-not-allowed opacity-70'
            : 'bg-[#1a9e7e] hover:bg-[#158a6c] hover:scale-105' }}">
                Valider
            </button>
        </div>
    @else

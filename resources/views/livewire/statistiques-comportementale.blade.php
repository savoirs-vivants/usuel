        ...
        <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2"
                    @if($timeRange !== 'Custom') style="display:none" @endif
                ...
                <div class="bg-gray-100 p-1 rounded-xl flex items-center gap-0.5">
                    @foreach (['J' => 'Jour', 'M' => 'Mois', 'A' => 'Année', 'Custom' => 'Custom'] as $range => $label)
                        <button wire:click="setTimeRange('{{ $range }}')" @click="setRange('{{ $range }}')"
            ...
            <div class="w-full min-w-0 p-6 bg-gray-50/40 overflow-hidden">
                ...
                <div class="mb-6 flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-mono font-bold text-sv-blue text-sm leading-none">
                            <span id="totalPassations">{{ $this->trackingData['total_passations'] }}</span> passations
                        </p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 mb-4">
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        <div class="flex items-center justify-between mb-5">
                            <div>
                                ...
                                <p class="font-bold text-sv-blue text-sm mt-0.5">Top 5 — Temps avant 1er clic (ms)</p>
                            </div>
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                        <div class="flex items-center justify-between mb-5">
                            <div>
                                <p class="text-xs font-bold text-gray-400 uppercase tracking-widest">Incertitude</p>
                                <p class="font-bold text-sv-blue text-sm mt-0.5">Top 5 — Taux de changement de réponse</p>
                            </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-4">
                    ...
                    <p class="text-xs text-gray-400 mb-5 leading-relaxed">
                        Évolution du temps de réponse selon la position de la question. Une courbe croissante indique un effet de fatigue ; décroissante, un effet d'entraînement.
                    </p>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    <div class="px-5 py-4 border-b border-gray-50 flex items-center justify-between">
                        ...
                        <span class="text-xs text-gray-400 font-medium">Moyennes sur toutes les passations filtrées</span>
                    </div>
                        ...
                        <table class="w-full text-xs min-w-[900px]">
                            <thead class="sticky top-0 z-10">
                                <tr class="bg-gray-50 border-b border-gray-100">
                                    ...
                                    <th class="text-right px-4 py-3 font-bold text-sv-blue uppercase tracking-wider whitespace-nowrap">Changements moy.</th>
                                    <th class="text-right px-4 py-3 font-bold text-sv-blue uppercase tracking-wider whitespace-nowrap">Hors-cible moy.</th>
                                    <th class="text-right px-4 py-3 font-bold text-sv-blue uppercase tracking-wider whitespace-nowrap">Pauses moy.</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-50">
                                @forelse ($this->trackingData['tableau'] as $row)
                                    <tr class="hover:bg-gray-50/80 transition-colors">
                                        <td class="px-4 py-2.5 font-mono font-bold text-gray-400">{{ $row['num'] }}</td>
                                        <td class="px-4 py-2.5">
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-lg bg-sv-green/10 text-sv-green font-bold text-xs">
                                                {{ $row['categorie'] }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-2.5 text-gray-600 max-w-xs truncate" title="{{ $row['intitule'] }}">
                                            {{ $row['intitule'] }}
                                        </td>
                                        @if ($row['nb_occurrences'] > 0)
                                            ...
                                            <td class="px-4 py-2.5 text-right font-mono">
                                                <span class="{{ $row['avg_changements'] > 0.5 ? 'text-orange-500 font-bold' : 'text-gray-700' }}">
                                                    {{ $row['avg_changements'] }}
                                                </span>
                                            </td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const CYAN = 'rgb(6,182,212)', CYAN_A = 'rgba(6,182,212,0.75)';
    const ORANGE = 'rgb(249,115,22)', ORANGE_A = 'rgba(249,115,22,0.75)';
    const SV_GREEN = 'rgb(26,158,126)';

    const tooltipBase = { backgroundColor:'#1a2340', titleColor:'#fff', bodyColor:'#9ca3af', borderColor:'#374151', borderWidth:1, padding:10, boxPadding:6 };
    const axisTicks = { color:'#9ca3af', font:{ size:11 }, padding:8 };

    function initCharts(data) {
        if (!data) return;
        if (latenceChart)    { latenceChart.destroy();    latenceChart    = null; }
        if (changementsChart){ changementsChart.destroy(); changementsChart= null; }
        if (ordreChart)      { ordreChart.destroy();       ordreChart      = null; }
        ...
        latenceChart = new Chart(document.getElementById('latenceChart'), {
            ...
            data: {
                ...
                datasets: [{ data: hasL ? data.top5_latence.map(d=>d.value) : [0], backgroundColor: CYAN_A, borderColor: CYAN, borderWidth:1.5, borderRadius:{topLeft:0,topRight:6,bottomLeft:0,bottomRight:6}, borderSkipped:'left', barPercentage:0.6 }]
            },
            options: { indexAxis:'y', responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, tooltip:{...tooltipBase, callbacks:{label:c=>` ${c.raw} ms`}} }, scales:{ x:{ grid:{display:false}, border:{display:false}, ticks:{...axisTicks, callback:v=>v+' ms'} }, y:{ grid:{display:false}, border:{display:false}, ticks:axisTicks } } }
        });
        ...
        changementsChart = new Chart(document.getElementById('changementsChart'), {
            ...
            data: {
                ...
                datasets: [{ data: hasC ? data.top5_changements.map(d=>d.value) : [0], backgroundColor: ORANGE_A, borderColor: ORANGE, borderWidth:1.5, borderRadius:{topLeft:6,topRight:6,bottomLeft:0,bottomRight:0}, borderSkipped:false, barPercentage:0.55 }]
            },
            options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, tooltip:{...tooltipBase, callbacks:{label:c=>` ${c.raw} changement(s) moy.`}} }, scales:{ x:{ grid:{display:false}, border:{display:false}, ticks:axisTicks }, y:{ grid:{color:'#f3f4f6'}, border:{display:false}, ticks:axisTicks, beginAtZero:true } } }
        });
        ...
        ordreChart = new Chart(document.getElementById('ordreChart'), {
            ...
            data: {
                ...
                datasets: [
                    { label:'Temps total moy. (ms)', data: hasO ? data.ordre_temps : [0], borderColor: SV_GREEN, fill:false, tension:0.35, pointRadius:3, pointHoverRadius:6, borderWidth:2 },
                    { label:'Latence moy. (ms)', data: hasO ? data.ordre_latence : [0], borderColor: CYAN, fill:false, tension:0.35, pointRadius:3, pointHoverRadius:6, borderWidth:2, borderDash:[4,3] },
                    { label:'Changements moy.', data: hasO ? data.ordre_changements : [0], borderColor: ORANGE, fill:false, tension:0.35, pointRadius:3, pointHoverRadius:6, borderWidth:2, borderDash:[2,3], yAxisID:'y2' }
                ]
            },
            options: {
                responsive:true, maintainAspectRatio:false, interaction:{mode:'index',intersect:false},
                plugins:{ legend:{ position:'top', align:'end', labels:{usePointStyle:true,pointStyle:'circle',font:{size:11},color:'#374151',padding:16} }, tooltip:tooltipBase },
                scales: {
                    x:{ grid:{display:false}, border:{display:false}, ticks:{...axisTicks, maxTicksLimit:15} },
                    y:{ grid:{color:'#f3f4f6'}, border:{display:false}, ticks:axisTicks, beginAtZero:true, title:{display:true,text:'ms',color:'#9ca3af',font:{size:10}} },
                    y2:{ position:'right', beginAtZero:true, grid:{display:false}, border:{display:false}, ticks:{...axisTicks,font:{size:10}}, title:{display:true,text:'changements',color:'#9ca3af',font:{size:10}} }
                }
            }
        });
